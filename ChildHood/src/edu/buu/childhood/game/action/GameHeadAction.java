...
public class GameHeadAction extends ActionSupport{
	...
	public String execute() throws Exception {
		...
		case C.def.APP_REQUEST:
			Page<GameHead> gameHeadPage=gameService.getGameHeadPage(gameArea, ageCode, memNumCode, pageNum);
			HttpSession session=ServletActionContext.getRequest().getSession();
			if(gameHeadPage!=null){
				Message<Page<GameHead>> message=new Message<Page<GameHead>>("Page<GameHead>",gameHeadPage);
				session.setAttribute("JsonString", json.toJson(message,new TypeToken<Message<Page<GameHead>>>(){}.getType()));
			}else{
				session.setAttribute("JsonString", json.toJson(new Message<String>("error","error"),new TypeToken<Message<String>>(){}.getType()));
			}
			return "MessagePage";
