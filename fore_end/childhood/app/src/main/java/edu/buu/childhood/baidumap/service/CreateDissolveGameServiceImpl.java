 ...
 */
public class CreateDissolveGameServiceImpl {
    Gson json = new Gson();
    public CallBackPage<BackItem> getCreateDissolveGameHeadInf(String result) {
        Message<String> message = json.fromJson(result, new TypeToken<Message<String>>() {
        }.getType());
        if (C.onekey.GAME_CANCELED_SUCCESS.equals(message.getMessageCode())) {
